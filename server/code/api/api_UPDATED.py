...
def _save_chat_memory_fs(conv_id: str, messages: List[Dict[str, Any]]) -> Dict[str, str]:
    """
    FS export (JSON/TXT/JSONL). DB is canonical; FS is a convenience export.
    """
...
def stt_transcribe(
    ...
    if kind == "none" or engine is None:
        raise HTTPException(501, "STT engine unavailable. Install faster-whisper and ffmpeg.")
...
def _load_model():
    ...
    model_id = (MODEL_ID or BASE_MODEL_ID).strip()
    ...
    print(f"[LLM] Loading {model_id} 4bit={LOAD_IN_4BIT} attn={ATTN_IMPL} dtype={TORCH_DTYPE}")
    ...
    init_db()
    return _model, _tokenizer
# ---------------- Schemas (hardened for Pro UI) ----------------
class ChatTurn(BaseModel):
    role: str
    content: str
class _ChatReq(BaseModel):
    """
    Hardened request: allow extra keys from Pro UI (meta/state/roadmap/thread objects)
    without triggering 422.
    """
...
# ---------------- REST chat (non-WS, still useful for testing) ----------------
@router.post("/chat")
def chat(req: _ChatReq) -> Dict[str, Any]:
    start = time.time()
    model, tok = _load_model()
    msgs = [{"role": _normalize_role(m.role), "content": m.content} for m in req.messages]
...
# ---------------- Streaming NDJSON (legacy, optional) ----------------
...
def chat_stream(req: _ChatReq):
    ...
    def gen():
        ...
        for delta in streamer:
            ...
            full += delta
            if stream_id:
                stream_bus.publish(stream_id, delta)
