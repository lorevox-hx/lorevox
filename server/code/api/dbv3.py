...
# =============================================================================
# Chat / session helpers (used by api.py + chat_ws.py)
# =============================================================================
...
def get_session(conv_id: str) -> Optional[Dict[str, Any]]:
    """Return chat session payload for a conv_id."""
    init_db()
    con = _connect()
    row = con.execute(
...
# =============================================================================
# People (used by routers/people.py)
# =============================================================================
def create_person(
    display_name: str,
...
def update_person(person_id: str, **fields: Any) -> None:
    """Patch-style update. Accepts display_name/role/date_of_birth/place_of_birth."""
    init_db()
...
# =============================================================================
# Profiles (used by routers/profiles.py)
# =============================================================================
...
def get_profile(person_id: str) -> Dict[str, Any]:
    """Returns row dict with profile_json already parsed into a dict."""
    init_db()
    con = _connect()
    row = con.execute(
...
def update_profile_json(
    ...
    new_doc = doc or {}
    if merge:
        merged = _deep_merge(existing, new_doc)
    else:
        merged = new_doc
...
def ingest_basic_info_document(person_id: str, document: Dict[str, Any], create_relatives: bool = False) -> None:
    """
    Convenience ingest for your basic-info.html-shaped JSON.
    Minimal behavior:
      - Merge into profile under "basic_info"
      - Optionally create relatives as People (best-effort) if create_relatives=True
    """
    if not isinstance(document, dict):
        return

    # Store whole form doc under a stable key
    update_profile_json(person_id, {"basic_info": document}, merge=True, reason="ingest_basic_info")

    if not create_relatives:
        return

    # Best-effort relative creation:
    # If the document contains arrays like relatives/kinship/etc, create people entries.
    # This stays intentionally loose so it won't break when your form evolves.
    candidates: List[Dict[str, Any]] = []
    for key in ("kinship", "relatives", "family", "relationships"):
        v = document.get(key)
    ...
    for item in candidates:
        ...
        if not name:
            continue
        # Avoid duplicates by naive lookup (same display_name)
        # (If you later add a relationships table, wire it here.)
        existing = [p for p in list_people(limit=1000, offset=0) if (p.get("display_name") or "").strip().lower() == name.lower()]
        if existing:
            continue
        create_person(
            display_name=name,
            role=item.get("relation") or item.get("role"),
            date_of_birth=item.get("date_of_birth") or item.get("dob"),
            place_of_birth=item.get("place_of_birth") or item.get("pob"),
        )


# =============================================================================
# Media (used by routers/media.py)
# =============================================================================
def add_media(
    person_id: str,
    file_path: str,
...
def rag_query(query: str, k: int = 5, only_ids: Optional[List[str]] = None) -> List[Dict[str, Any]]:
    ...
    Lightweight lexical scoring (fast, no heavy deps):
      score = number of query tokens found in chunk text + small title boost
    """
    init_db()
    con = _connect()
