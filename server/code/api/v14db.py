...
DB_PATH = DB_DIR / "lorevox.sqlite3"
...
def ingest_basic_info_document(person_id: str, text: str) -> Dict[str, Any]:
    """
    Minimal stub used by router: stores document text under profile_json['ingest']['basic_info'].
    (Later you can replace with an LLM extractor.)
    """
    init_db()
...
# -----------------------------------------------------------------------------
# RAG minimal (db_inspector may call these)
# -----------------------------------------------------------------------------
