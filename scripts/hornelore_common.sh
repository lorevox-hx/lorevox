...
# Hornelore must use its own launcher copies under hornelore/launchers,
# not the shared parent Lorevox launchers.
...
kill_all_hornelore() {
  local killed=0
  for pattern in "run_gpu_8000" "uvicorn.*8000" "run_tts_8001" "uvicorn.*8001" "hornelore-serve"; do
    if pgrep -f "$pattern" >/dev/null 2>&1; then
      printf 'Killing stale process: %s\n' "$pattern"
      pkill -f "$pattern" 2>/dev/null || true
      killed=1
